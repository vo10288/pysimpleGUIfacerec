...
def perform_face_recognition(image_1, image_2):
    ...
    width = min(image_1_rgb.shape[1], image_2_rgb.shape[1])
    height = min(image_1_rgb.shape[0], image_2_rgb.shape[0])
    image_1_rgb = cv2.resize(image_1_rgb, (width, height))
    image_2_rgb = cv2.resize(image_2_rgb, (width, height))
    ...
    if len(face_locations_1) > 0 and len(face_locations_2) > 0:
        ...
        matches = face_recognition.compare_faces([image_1_rgb], image_2_rgb)[0]

        # Restituisce il risultato del confronto e le immagini modificate
        return matches, image_1, image_2

    # Restituisce il risultato del confronto e le immagini originali
    return None, image_1, image_2
