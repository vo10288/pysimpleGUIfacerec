...
def perform_face_recognition(image_1, image_2):
    ...
    # Ridimensiona le immagini in base alla dimensione dell'immagine più piccola
    min_height = min(image_1_rgb.shape[0], image_2_rgb.shape[0])
    min_width = min(image_1_rgb.shape[1], image_2_rgb.shape[1])
    image_1_rgb_resized = cv2.resize(image_1_rgb, (min_width, min_height))
    image_2_rgb_resized = cv2.resize(image_2_rgb, (min_width, min_height))

    # Trova i volti nelle immagini
    face_locations_1 = face_recognition.face_locations(image_1_rgb_resized)
    face_locations_2 = face_recognition.face_locations(image_2_rgb_resized)

    if len(face_locations_1) > 0 and len(face_locations_2) > 0:
        # Calcola i landmark del viso
        face_landmarks_1 = face_recognition.face_landmarks(image_1_rgb_resized, face_locations_1)
        face_landmarks_2 = face_recognition.face_landmarks(image_2_rgb_resized, face_locations_2)
        ...
        # Ridimensiona le immagini originali per visualizzare correttamente i risultati
        image_1_modified = cv2.resize(image_1, (min_width, min_height))
        image_2_modified = cv2.resize(image_2, (min_width, min_height))
        draw_match_rectangle(image_1_modified, face_locations_1[0], image_2_modified, face_locations_2[0])

    # Restituisce le immagini modificate
    return image_1_modified, image_2_modified
    ...
    if event == sg.WINDOW_CLOSED:
        ...
        if image_1 is not None and image_2 is not None:
            try:
                ...
                # face location and encodings
                
                #face_locationssx = face_recognition.face_locations(image_1)
                #face_locationsdx = face_recognition.face_locations(image_2)
                
                #face_encodingssx = face_recognition.face_encodings(image_1, face_locationssx)[0]
                #face_encodingsdx = face_recognition.face_encodings(image_2, face_locationsdx)[0]
                # Esegue il confronto facciale
                
                matches = face_recognition.compare_faces([image_1_modified], image_2_modified)[0]
                distances = face_recognition.face_distance([image_1_modified], image_2_modified)[0]
                ...
                print(str(distances[0][0])+' %')
                #print(str(face_locationssx))
                #print(str(face_locationsdx))
                #print(str(face_encodingssx))
                #print(str(face_encodingsdx))

                # Salva il risultato in un file CSV
                timestamp = datetime.now().strftime('%Y-%m-%d_%H-%M-%S')
                report_file = f'reports/{timestamp}.csv'
                with open(report_file, 'w') as file:
                    file.write('Immagine 1,Immagine 2,Risultato, distanza biometrica\n')
                    
                    file.write(f'{image_path_1},{image_path_2},{matches[0][0]},{distances[0][0]}\n')
                    
                    #file.write(f'FACE LOCATION FIRST IMAGE SX\n')
                    #file.write(f'str({face_locationssx})\n')
                    #file.write(f'FACE ENCODING FIRST IMAGE SX\n')
                    #file.write(f'str({[image_1_modified]})\n')
                    
                    #file.write(f'FACE LOCATION FIRST IMAGE DX\n')
                    #file.write(f'str({face_locationsdx})\n')
                    #file.write(f'FACE ENCODING FIRST IMAGE DX\n')
                    #file.write(f'str({[image_2_modified]})\n')
