...
layout = [
    [sg.Image(key='image_left1'),sg.Image(key='image_left'), sg.Image(key='image_right1'), sg.Image(key='image_right')],
    [sg.Button('Seleziona immagine sinistra', button_color=('white', 'darkblue'), size=(25, 1)), sg.Button('Seleziona immagine destra', button_color=('white', 'darkblue'), size=(25, 1)), sg.Button('Face Recognition', button_color=('white', 'darkblue'), size=(20, 1)), sg.Button('OpenTimestamps-Client', button_color=('white', 'darkblue'), size=(25, 1)), sg.Button('Stampa finestra', button_color=('white', 'darkblue'), size=(15, 1))],
    [sg.Output(size=(120, 25), key='output', font=('Helvetica', font_size), background_color=background_color, text_color=text_color)]
]
		...
		try:
			...
			#######################################
				...
				if width > 250 or height > 250:
					...
					scale = max(250 / width, 250 / height)
				...
				if image_1 is not None:
					image_11_bytes = cv2.imencode('.png', image_1)[1].tobytes()
					window['image_left1'].update(data=image_11_bytes)
			...
			print('image on the left not reloaded!!')
		...
		try:
            ...
            #######################################
				...
				if image_2 is not None:
					image_22_bytes = cv2.imencode('.png', image_2)[1].tobytes()
					window['image_right1'].update(data=image_22_bytes)
			...
			print('image on the right not reloaded!!')
...
#first_image_encodeds, second_image_encodeds, face_landmarks_1, face_landmarks_1, face_locations_1, face_locations_2, matches, distances, image_1, image_2                    
	...
	elif event == 'OpenTimestamps-Client':
		...
		try:
			...
			if platform.system() == 'macOS': #Ventura': #Darwin':  # macOS
				command = f'screencapture -w reports/{timestamp}.png'
				subprocess.run(command, shell=True)
		
			elif platform.system() == 'Linux':  # Linux
				command = f'import -window root reports/{timestamp}.png'
				subprocess.run(command, shell=True)
		
			elif platform.system() == 'Windows':  # Windows
				command = f'powershell -c "Add-Type -TypeDefinition \'using System; using System.Runtime.InteropServices; public class PInvoke {{ [DllImport(\\"user32.dll\\")] public static extern IntPtr GetDesktopWindow(); [DllImport(\\"user32.dll\\")] public static extern bool PrintWindow(IntPtr hwnd, IntPtr hDC, uint nFlags); }} public class Program {{ public static void Main() {{ var hwnd = PInvoke.GetDesktopWindow(); var bmp = new System.Drawing.Bitmap(800, 600); using (var g = System.Drawing.Graphics.FromImage(bmp)) {{ var hdc = g.GetHdc(); PInvoke.PrintWindow(hwnd, hdc, 0); g.ReleaseHdc(hdc); }} bmp.Save("reports/{timestamp}.png"); }} }}"'
				subprocess.run(command, shell=True)
			else:
				command = f'screencapture -w reports/{timestamp}.png'
				subprocess.run(command, shell=True)
				sg.popup('Sistema operativo non supportato.', title='Errore')
			
			sg.popup('Screenshot catturato e salvato come file PNG', title='Successo')
	
		except Exception as e:
			sg.popup('Si è verificato un errore durante la cattura dello screenshot.', title='Errore')
